...
router.get("/", async (req, res) => {
  try {
    const { page = 1, limit = 10, sort = "new", subreddit, tag, search } = req.query

    let posts

    if (search) {
      posts = await Post.searchPosts(search, { page, limit })
    } else if (subreddit) {
      const subredditDoc = await Subreddit.findOne({ name: subreddit })
      if (!subredditDoc) {
        return res.status(404).json({ message: "Subreddit not found" })
      }
      posts = await Post.findBySubreddit(subredditDoc._id, { page, limit, sort })
    } else if (tag) {
      ...
      posts = await Post.findForFeed({ page, limit, sort })
    }
  ...
  try {
    ...
    if (tags) {
      try {
        ...
        processedTags = processedTags.map((tag) => tag.toString().toLowerCase())
      } catch (e) {
...
router.put("/:id", protect, async (req, res) => {
  try {
    ...
    if (post.isLocked) {
      return res.status(403).json({ message: "This post is locked and cannot be edited" })
    }
